      ...
      (
        {
          ...
          modules-right =
            lib.optionals osConfig.etu.graphical.window-managers.voxtype.enable [
              "custom/voxtype"
            ]
            ++ [
              "privacy"
            ...
            exec = lib.getExe (
              pkgs.writeShellApplication {
                ...
                runtimeInputs = [
                  ...
                  pkgs.gnugrep
                  pkgs.gnused
                  osConfig.services.upower.package
                ];
                text = ''
        ...
        // lib.optionalAttrs osConfig.etu.graphical.window-managers.voxtype.enable {
          "custom/voxtype" = {
            exec = lib.getExe (
              pkgs.writeShellApplication {
                name = "voxtype-waybar-status";
                runtimeInputs = [
                  pkgs.voxtype-vulkan
                  pkgs.jq
                ];
                text = ''
                  voxtype status --follow --format json --extended \
                    | jq --unbuffered -c 'if (.class == "idle" or .class == "stopped") then .text = "" else . end'
                '';
              }
            );
            return-type = "json";
