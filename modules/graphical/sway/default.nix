  ...
  config = lib.mkIf config.etu.graphical.sway.enable {
    ...
    home-manager.users.${config.etu.user.username} = lib.mkIf config.etu.user.enable {
      ...
      services.swayidle = {
        enable = true;
        timeouts = [
          { timeout = 300; command = config.etu.graphical.sway.lockCommand; }
          { timeout = 600; command = "${config.etu.graphical.sway.package}/bin/swaymsg 'output * dpms off'"; }
        ];
        events = [
          { event = "after-resume"; command = "${config.etu.graphical.sway.package}/bin/swaymsg 'output * dpms on'"; }
          { event = "before-sleep"; command = config.etu.graphical.sway.lockCommand; }
        ];
      }; # END swayidle
