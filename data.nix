{
  # This file contains both public data, but also some dummy secret
  # data... and a loader to replace the dummy secret data with the
  # actual secrets from an actually encrypted and secret file.
  secrets = let
    # If we have a locally defined secrets file, let's import it to
    # overwrite the dummy default secrets.
    dataSecrets = if builtins.pathExists ./data-secrets.nix then import ./data-secrets.nix else {};
  in {
    # Dummy passwords to use for accounts, remember to create a
    # data-secrets.nix with newly generated passwords using the
    # following command:
